...
class DashboardController {
  ...
  static PremiumUser = async (req, res) => {
    ...
    const _list = list.map(async (e) => {
      const tenure = await ReferralReportModel.findOne({
        freemember_tgid: { $eq: e.tgid },
      });
      return {
        ...e,
  ...
  static MembershipTenure = async (req, res) => {
    ...
    res.render("Configuration/Membership", {
      baseUrl,
  ...
  static SystemImages = async (req, res) => {
    ...
    res.render("Configuration/Systemimage", {
      baseUrl,
      data: {},
      system: system,
      path: "system",
  ...
  static Folder = async (req, res) => {
    ...
    res.render("Configuration/Folder", {
      baseUrl,
      folder: folder,
      path: "folder",
      session: req.session,
  ...
  static Paypal = async (req, res) => {
    res.render("MemberPayment/Paypal", {
      baseUrl,
      toncoin: [],
      path: "paypal",
      session: req.session,
  ...
  // .......................PAYMENT[USDT].............................
  static USDTPayment = async (req, res) => {
    const usdtPayments = await USDTMembershipPaymentModel.find({}).sort({
      createdAt: -1,
    });
    let list = usdtPayments.map(async (e) => {
      ...
      let formattedDate = e.date
        ? moment(e.date, ["YYYY-MM-DD", "DD-MM-YYYY"]).format("DD-MM-YYYY")
        : "";
      return {
        _id: e._id,
        user: e.user,
    ...
    Promise.all(list).then((result) => {
      res.render("MemberPayment/USDTMembershipPayment", {
        baseUrl,
        usdtPayments: result,
        path: "usdtmembership",
        session: req.session,
  ...
  static TelegramCoinPayment = async (req, res) => {
    ...
    let list = telegramCoinPayments.map(async (v) => {
      ...
      let membership = await MembershipModel.findById(v.membership_id);
      // Format date as dd-mm-yyyy
      let formattedDate = v.date
        ? moment(v.date, ["YYYY-MM-DD", "DD-MM-YYYY"]).format("DD-MM-YYYY")
        : "";
      return {
        _id: v._id,
        user: v.user,
        ...
        username: userdetail?.username,
        tenure: membership?.membershiperiod,
        amount: v.amount, // Coin amount
        date: formattedDate,
      };
    });
    Promise.all(list).then((result) => {
      res.render("MemberPayment/TelegramCoinMembershipPayment", {
        baseUrl,
        telegramCoinPayments: result,
        path: "telegramcoinmembership",
        session: req.session,
      });
    });
  };
  ...
  static EditLogo = async (req, res) => {
    ...
    res.render("Configuration/EditLogo", {
      baseUrl,
      logo: logo,
      path: "logo",
      loginUser: req.user,
  ...
  static ReferralMembershipTenure = async (req, res) => {
    let referralmembership = await ReferralMembershipModel.find({}).sort({
      _id: -1,
    });
    res.render("Referral/Referralmembership", {
      baseUrl,
  ...
  static ReferralDetail = async (req, res) => {
    let referraldetail = await ReferralReportModel.find({
      referral_user_id: req.params.id,
    }).sort({ _id: -1 });
    res.render(`Referral/Referraldetail`, {
      baseUrl,
  ...
  static ReferralReport = async (req, res) => {
    ...
    Promise.all(referral).then((resp) => {
      res.render("Referral/Referralreport", {
        baseUrl,
        referral: resp,
        path: "referralreport",
        session: req.session,
  ...
  static PaymentConfiguration = async (req, res) => {
    ...
    res.render("Configuration/PaymentConfiguration", {
      baseUrl,
  ...
  static AddPaymentConfiguration = async (req, res) => {
    ...
    res.render("Configuration/AddPaymentConfiguration", {
      baseUrl,
      addpaymentconfiguration: addpaymentconfiguration,
      path: "addpaymentconfiguration",
      session: req.session,
    });
  };
  ...
  static MembershipStripePayment = async (req, res) => {
    let membershipstripe = await MembershipStrpiePaymentModel.find({}).sort({
      createdAt: -1,
    });
    ...
    Promise.all(list).then((result) => {
      res.render("MemberPayment/MembershipStripePayment", {
        baseUrl,
        data: {},
        membershipstripe: result,
        path: "membershipstripe",
        session: req.session,
      });
    });
  };
  ...
  static AdminUser = async (req, res) => {
    ...
    res.render("MasterAdmin/Adminuser", {
      baseUrl,
